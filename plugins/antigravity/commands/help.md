...
First run this, so the task types and model aliases come from the plugin itself rather than from this file:

```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/agy-companion.mjs" types
```

Then show the user the reference below, filling the task-type table and the model alias list from that output — `taskTypes` and `modelAliases`. Never list either from memory; that is exactly how this card went stale before. Keep it a compact cheat sheet, not the manual. Do not add commentary, and do not run any other command.
...
**Task types** — from the `taskTypes` output above. Name one as the first word, or leave it out and it is inferred from what you asked for.

**Model aliases** — from the `modelAliases` output above, shown as `alias → slug`.
...
| `--model <name>` | An alias from the `modelAliases` output above, or any full slug from `agy models`. |
